...
function formatDataForChart(chartData, chartType) {
  ...
  const labels = dataPoints.map(point => point.label || point.name || 'Unknown');
  const values = dataPoints.map(point => point.value || point.amount || 0);
  
  const datasets = [{
    data: values,
    backgroundColor: chartType === 'line' || chartType === 'area' ? CHART_COLORS.primary : [
    ...
    borderColor: chartType === 'line' || chartType === 'area' ? CHART_COLORS.primary : 'rgba(255, 255, 255, 0.8)',
            ...
            if (data.labels.length && data.datasets.length) {
              ...
              return data.labels.map((label, i) => {
                ...
                if (chartType === 'pie' || chartType === 'doughnut') {
                  ...
                  // For other chart types, show value
                ...
                return {
                  ...
                  pointStyle: 'circle',
                  hidden: isNaN(dataset.data[i]) || chart.getDatasetMeta(0).data[i]?.hidden === true,
                  index: i
                };
              });
            }
...
function createChartElement(chartConfig, confidence) {
  ...
  const title = chartConfig.title || chartConfig.chartTitle || 'Data Visualization';
  const description = chartConfig.description || chartConfig.analysisDescription || 'AI-generated chart based on your data';
...
function downloadChartAsImage(canvas, title) {
  try {
    ...
    tempCtx.drawImage(canvas, 0, 0, originalWidth * scale, originalHeight * scale);
...
function handleChartModeSelection(event) {
  ...
  const button = event.currentTarget.closest('.chart-mode-btn') || event.currentTarget;
...
function displayResults(data) {
  ...
  if (data.multiChartAnalysis) {
    ...
    data.chartConfigs.forEach((chartConfig, index) => {
      const confidence = data.multiChartAnalysis.confidenceScores?.[index] || 0.9;
      const chartCard = createChartElement(chartConfig, confidence);
      chartsContainer.appendChild(chartCard);
      
      const canvas = chartCard.querySelector('.chart-canvas');
      setTimeout(() => {
        const title = chartConfig.title || chartConfig.chartTitle || `Chart ${index + 1}`;
        
        // For multi-chart, analyzedData might be in chartConfig directly or nested
        const analyzedData = chartConfig.analyzedData || chartConfig.dataPoints || chartConfig;
        ...
        downloadImgBtn.addEventListener('click', () => downloadChartAsImage(canvas, title));
      }, index * 200);
    });
  } else {
    ...
    const canvas = chartCard.querySelector('.chart-canvas');
    setTimeout(() => {
      const title = chartConfig.title || chartConfig.chartTitle || data.analyzedData?.title || 'Data Visualization';
...
async function downloadAllCharts() {
  try {
    ...
    showNotification(`${canvases.length} high-quality charts downloaded successfully!`, 'success');
